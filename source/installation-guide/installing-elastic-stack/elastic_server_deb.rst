...
Elasticsearch is a highly scalable full-text search and analytics engine. More info on `Elastic <https://www.elastic.co/products/elasticsearch>`_.
...
3. **Optional.** Kibana will listen only on the loopback interface (localhost) by default. To set up Kibana to listen on all interfaces, edit the file ``/etc/kibana/kibana.yml``. Uncomment the setting ``server.host`` and change the value to::
  ...
  .. note::
    ...
    It is recommended to set up an Nginx proxy for Kibana to use SSL encryption and to enable authentication. Instructions to set this up can be found at :ref:`kibana_ssl`.
